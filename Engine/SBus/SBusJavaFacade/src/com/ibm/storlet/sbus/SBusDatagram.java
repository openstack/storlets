...
public class SBusDatagram {
	...
	public static enum eStorletCommand {
		SBUS_CMD_HALT(0), SBUS_CMD_EXECUTE(1), SBUS_CMD_START_DAEMON(2), SBUS_CMD_STOP_DAEMON(
				3), SBUS_CMD_DAEMON_STATUS(4), SBUS_CMD_STOP_DAEMONS(5), SBUS_CMD_PING(
				6), SBUS_CMD_DESCRIPTOR(7), SBUS_CMD_CANCEL(8), SBUS_CMD_NOP(9);
	...
	public static enum eFileDescription {
		SBUS_FD_INPUT_OBJECT(0), SBUS_FD_OUTPUT_OBJECT(1), SBUS_FD_OUTPUT_OBJECT_METADATA(
				2), SBUS_FD_OUTPUT_OBJECT_AND_METADATA(3), SBUS_FD_LOGGER(4), SBUS_FD_OUTPUT_CONTAINER(
				5), SBUS_FD_OUTPUT_TASK_ID(6);
	...
	private void setFilesMetadataFromJSON(String strJSONMetadata) {
		...
		HashMap<String, String>[] FilesMetadata = (HashMap<String, String>[]) new HashMap[getNFiles()];

		HashMap<String, String> HashOfHashes = convertJSONtoMap(strJSONMetadata);
		for (int i = 0; i < getNFiles(); ++i) {
			String strIdx = new String() + i;
			String strCurrJSONMetadata = HashOfHashes.get(strIdx);
			HashMap<String, ?> ObtainedMetadata = convertJSONtoMap(strCurrJSONMetadata);
	...
	private HashMap<String, String> convertJSONtoMap(String strJSON) {
		...
		try {
			@SuppressWarnings("unchecked")
			HashMap<String, String> r = (HashMap<String, String>) new JSONParser()
					.parse(strJSON, new ContainerFactory() {
